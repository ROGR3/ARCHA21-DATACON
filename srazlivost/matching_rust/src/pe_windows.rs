...
/// Same bucketing as `compute_pe_map`, but without the cohort-eligibility gate
/// on anchor dates. Used for the booster analysis, where there is no vaccine
/// rollout window to respect (booster eligibility isn't age-cohort-gated the
/// way the first dose rollout was).
pub fn compute_pe_map_unrestricted(
    persons: &[Person],
    indices: &[usize],
    anchor_dates: &[NaiveDate],
    config: &Config,
    ...
    eprintln!(
        "  pe_map (unrestricted): processing {total} persons across {} threads...",
        rayon::current_num_threads(),
    );
    ...
    let map = indices
        .par_iter()
        .fold(PeMap::new, |mut map, &pidx| {
            process_person_unrestricted(&mut map, persons, pidx, anchor_dates, ref_year, inj_mode);
            map
        })
        .reduce(PeMap::new, |mut a, b| {
            merge_maps(&mut a, b);
    ...
    eprintln!("  pe_map (unrestricted): done ({total} persons)");
    map
}

fn process_person_unrestricted(
    map: &mut PeMap,
    persons: &[Person],
    pidx: usize,
    anchor_dates: &[NaiveDate],
